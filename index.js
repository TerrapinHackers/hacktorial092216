...
login({email: config.username, password: config.password}, function callback (err, api) {
    ...
    api.listen(function callback(err, message) {
        ...
        if (message.type == 'message'){
            ...
            // Initialize color and iterate through the word array looking for
            // a color.
            var color;
            wordArray.forEach(function (word){
                /* !color: Makes sure we haven't already found a color. This
                 * means that the counted color will be the first one.
                 * word in colours: Checking if the word is in the massive dict
                 * of colours.
                 */
                if (!color && word in colours){
            ...
            // If we found a color:
            if (color){
                console.log("Desired hex: " + colours[color]);
                api.changeThreadColor(colours[color], message.threadID, function callback(err) {
                    if(err) return console.error(err);
